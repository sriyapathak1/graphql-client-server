...
exports.validateUser = exports.validatorUserRole = exports.validateEmail = void 0;

var _MockData = require("../lib/MockData");

const validateEmail = email => {
  var regx = /(\W|^)[\w.+\-]{5,}@successive\.tech(\W|$)/; // console.log("regx.test(email)",regx.test(email));
...
const validatorUserRole = (email, role) => {
  const x = _MockData.MockEmailRole.filter(m => {
...
exports.validatorUserRole = validatorUserRole;
const Arr = [];
const ArrInvalid = [];

const validateUser = () => {
  _MockData.MockDdata.map(field => {
...
console.log('validate function', validateUser()); //  const validateUser = () => {
